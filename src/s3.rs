...
) -> Result<S3Client, Box<dyn Error + Sync + Send>> {
...
pub(crate) async fn create_bucket(client: &S3Client, name: &str) -> Result<(), Box<dyn Error + Sync + Send>> {
...
pub(crate) async fn remove_bucket(client: &S3Client, bucket: &str) -> Result<(), Box<dyn Error + Sync + Send>> {
